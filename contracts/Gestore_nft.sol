...
contract Gestore_nft is Ownable {
    ...
    function trasferimentoMateriaPrima(
        address _destinatario, uint256 _lotto, address _contract
        ) external {  
        ...
        carbonFootprint.approve(_contract, risorsa.token);
        carbonFootprint.safeTransferFrom(msg.sender, _destinatario, risorsa.token);
    }
    ...
    function creaProdotto(
        string[] memory _nomi_attivita, uint32[] memory _valori_CO2_attivita, string memory _nome, uint256[] memory _lotti_materie_prime, uint256 _lotto
    ) external {

        // errore se il lotto del prodotto da creare esiste già 
        require(carbonFootprint.getRisorsaByLotto(_lotto).exists==false, "Il lotto inserito \xc3\xa8 gi\xC3\xa0 presente");
        ...
        carbonFootprint.creaProdotto(_lotto, CO2_totale, _nomi_attivita, _valori_CO2_attivita, "prodotto trasformato", msg.sender, _lotti_materie_prime, _nome);
    ...
    function getInfoByToken(
        uint256 _token
    ) public view returns(string memory, uint256, string[] memory, uint32[] memory, uint32, string memory, 
